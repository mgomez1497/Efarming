...
namespace WebApplication1.Controllers
{
    public class PlantsController : ApiController
    {
        ...
        [HttpGet]
        [Route("api/Plants")]
        public IHttpActionResult GetPlants(string productivityId)
        {
            var plants = db.Plants.Where(p => p.ProductivityId.ToString() == (productivityId)).Select(p => new
            {
                p.Id,
                p.Hectares,
        ...
        [HttpPut]
        [Route("api/Plants/PutPlants/{id}")]

        public IHttpActionResult PutPlants(Guid id, Plants plant)
        {
            ...
            try
            {
                var plants = db.Plants.Where(p => p.Id == id);
